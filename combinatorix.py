...
take0 = apply(lambda x: x[0])


parenthese_open = char("(")
parenthese_close = char("(")
phrase = join(sequence(char('"'), join(one_or_more(unless(char('"'), anything))), char('"')))
word = join(one_or_more(unless(space, anything)))
term = either(phrase, word, parenthese_open, parenthese_close)
query_parser = one_or_more(take0(sequence(term, either(join(one_or_more(space)), end_of_stream))))
